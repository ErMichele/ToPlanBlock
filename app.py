...
from flask import (
    Flask, render_template, redirect, send_file,
    url_for, request, flash, session, g
)
...
app.config['SQLALCHEMY_DATABASE_URI'] = (
    os.getenv('PRODUCTION_DATABASE_URL') if IS_PROD
    else os.getenv('DEV_DATABASE_URL', 'sqlite:///todo.db')
)
...
class User(db.Model, UserMixin):
    ...
    def profile_pic_url(self):
        """Reconstructs the full Cloudinary URL from the stored filename."""
...
# Association table for the many-to-many Task ↔ Category relationship.
# todo_id has no standalone index: the primary key (todo_id, category_id) already covers it.
# category_id gets a standalone index to speed up reverse lookups (find all tasks for a category).
...
class Category(db.Model):
    ...
    color      = db.Column(db.String(7), nullable=True)   # hex string, e.g. '#ff6347'; NULL → use theme colour
...
class Todo(db.Model):
    ...
    __table_args__ = (
        # Compound index covers the three most common filter combinations on the task list
...
def allowed_file(filename):
    """Returns True if the filename carries a permitted image extension."""
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS


def parse_categories_string(csv_string):
    """Converts a comma-separated category string into a deduplicated list of uppercase names."""
    if not csv_string:
        return []
...
def toggle_category_string(current_str, toggle_name):
    """Adds or removes a category name from a comma-separated filter string."""
    if not current_str:
        return toggle_name
...
def cleanup_unlocked_categories(user_id):
    """Deletes any unlocked, empty categories for the given user in a single query."""
...
@app.before_request
def set_nonce():
    """Generates a per-request CSP nonce used to whitelist inline scripts."""
    g.nonce = secrets.token_urlsafe(16)


@app.after_request
def add_security_headers(response):
    """Attaches security headers to every outgoing response."""
...
def render_markdown(text):
    ...
    Uses bleach to strip any tags not in the allow-list, preventing XSS
    even if user input contains raw HTML.
    """
    if not text:
        return ''
    ...
    return Markup(bleach.clean(html_content, tags=allowed_tags, attributes=allowed_attrs))
...
def request_entity_too_large(error):
    flash('The file is too large. Maximum allowed size is 5 MB.', 'danger')
    return redirect(url_for('account'))


@app.errorhandler(429)
def ratelimit_handler(error):
    limit_info = error.description if hasattr(error, 'description') else 'a few minutes'
    return render_template('429.html', limit_info=limit_info), 429
...
@limiter.exempt
def health_check():
    """Lightweight liveness probe for uptime monitors; exempt from rate limiting."""
    return 'OK', 200
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and bcrypt.check_password_hash(user.password, pw):
            ...
            # Reject open redirects: only follow the 'next' param if it's a relative path
...
# ── Task routes ────────────────────────────────────────────────────────────────
def _redirect_to_todo():
    """Convenience helper: redirects back to /todo preserving all active filter params."""
...
def _ajax_or_flash(message, category='info'):
    """Returns a JSON response for AJAX calls or flashes and redirects for standard form posts."""
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return {'status': 'success', 'message': message}, 200
    flash(message, category)
    return _redirect_to_todo()
...
def todo():
    if request.method == 'POST':
        ...
        if not task_text:
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return {'status': 'error', 'message': 'Task name is required.'}, 400
        ...
        if cat_list:
            ...
            for clean_name in cat_list:
                cat = existing_cats.get(clean_name) or Category(
                    name=clean_name, user_id=current_user.id, color=None
                )
                if not existing_cats.get(clean_name):
                    db.session.add(cat)
    ...
    # selectinload eliminates N+1 queries by batch-loading categories for all returned tasks
    ...
    if selected_category_input:
        for cat_name in parse_categories_string(selected_category_input):
            q = q.filter(Todo.categories.any(Category.name == cat_name))

    # Incomplete tasks first, then newest within each group
    ...
    completed_count       = q.filter(Todo.completed == True).count() if total_filtered_tasks > 0 else 0
    progress_percent      = int(completed_count / total_filtered_tasks * 100) if total_filtered_tasks > 0 else 0

    # ── Category sidebar query ────────────────────────────────────────────────
    ...
    if cat_sort_pref == 'alpha':
        ...
        # Default 'amount': sort by task count descending, then name for ties
        categories_query = (
            base_q.outerjoin(Category.todos)
    ...
    # Lightweight projections for the modal dropdowns — avoids pulling notes/timestamps
...
def toggle(todo_id):
    ...
    if session.get('auto_delete') and t.completed:
        ...
        return _ajax_or_flash('Task completed and auto-deleted.', 'info')

    db.session.commit()
    return _ajax_or_flash('Task updated.', 'info')
...
def delete(todo_id):
    ...
    db.session.commit()
    cleanup_unlocked_categories(current_user.id)
    return _ajax_or_flash('Task deleted.', 'info')
...
def edit(todo_id):
    ...
    if not task_text:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return {'status': 'error', 'message': 'Task name cannot be empty.'}, 400
        flash('Task name cannot be empty.', 'danger')
        return _redirect_to_todo()

    t.task  = task_text
    t.notes = notes_text or None
    ...
    new_categories = []
    for clean_name in cat_list:
        cat = existing_cats.get(clean_name) or Category(name=clean_name, user_id=current_user.id, color=None)
        if not existing_cats.get(clean_name):
            db.session.add(cat)
    ...
    return _ajax_or_flash('Task updated.', 'success')
...
def bulk_action():
    ...
    return _ajax_or_flash(msg, 'info')
...
def add_category():
    ...
    return redirect(url_for('todo', category=shown_category,
                            page=request.args.get('page', 1),
                            search=request.args.get('search', ''),
                            status=request.args.get('status', 'all'),
                            cat_page=request.args.get('cat_page', 1)))
...
def edit_category(cat_id):
    ...
    conflict = Category.query.filter_by(name=name, user_id=current_user.id).first()
    if conflict and conflict.id != cat.id:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return {'status': 'error', 'message': 'Another category already has this name.'}, 400
...
def delete_category(cat_id):
    ...
    updated_filter = toggle_category_string(request.args.get('category', ''), cat.name)
    return redirect(url_for('todo', category=updated_filter,
                            page=request.args.get('page', 1),
                            search=request.args.get('search', ''),
                            status=request.args.get('status', 'all'),
                            cat_page=request.args.get('cat_page', 1)))
...
def toggle_category_lock(cat_id):
    ...
    msg = f"Category '{cat.name}' {'locked' if cat.is_locked else 'unlocked'}."
    return _ajax_or_flash(msg, 'info')
    ...
    if request.method == 'POST':
        ...
        if 'picture' in request.files:
            ...
            if file and file.filename and allowed_file(file.filename):
                ...
                current_user.profile_pic_id = upload_result.get('public_id').split('/')[-1]

                # Delete the previous avatar from Cloudinary after a successful upload
                if old_id:
                    cloudinary.uploader.destroy(f"{target_folder}/{old_id}", invalidate=True)
...
def update_preferences():
    """Persists UI preferences in the server-side session."""
...
def delete_account():
    if current_user.profile_pic_id:
        folder = f"ToPlanBlock/{os.getenv('BRANCH', 'dev')}/profile_pics"
        cloudinary.uploader.destroy(f"{folder}/{current_user.profile_pic_id}", invalidate=True)
...
def export_tasks():
    """Exports all tasks for the current user as JSON or CSV."""
    ...
    filename_base = f"{current_user.username}_tasks_{datetime.now().strftime('%Y%m%d')}"
    ...
    if export_format == 'csv':
        ...
        for t in user_todos:
            writer.writerow([
                ...
                ', '.join(c.name for c in t.categories),
                t.notes or '',
            ])
    ...
    buffer = io.BytesIO(json.dumps(export_data, indent=4).encode('utf-8'))
